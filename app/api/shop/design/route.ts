...
const schema = z.object({
  ...
  filterBgColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).or(z.literal("rgba(0,0,0,0.08)")).or(z.string().startsWith("rgba")),
  filterTextColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  filterBorderColor: z.string(),
  sortBgColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  sortTextColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  sortBorderColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  reviewCountFontSize: z.number().int().min(10).max(20),
  reviewTitleColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  reviewBodyColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  reviewMetaColor: z.string().regex(/^#[0-9a-fA-F]{6}$/),
