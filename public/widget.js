...
<div class="rv-card" style="background:${cardBg};color:${design.textColor};border-radius:${r}px;padding:20px;font-size:${design.reviewTextSize}px;border:1px solid rgba(0,0,0,.06);box-shadow:0 1px 4px rgba(0,0,0,.05);${design.displayStyle==='carousel'?'min-width:260px;max-width:300px;flex-shrink:0;':''}">
  <div style="display:flex;align-items:flex-start;gap:14px;">
    ...
    function buildMain() {
      ...
      // ── A: MODERN CARD (Free+) — orange box + stars left, bars right, button
      const summaryModern = summary.total ? (function(){
      ...
      const summaryCompact = summary.total ? (function(){
      ...
      const summarySidebar = summary.total ? (function(){
      ...
      // ── D: HORIZONTAL BAR (Pro) — score box left + horizontal bars per star (5→1)
      const summaryHorizontal = summary.total ? (function(){
        var bars = summary.breakdown.map(function(b){
