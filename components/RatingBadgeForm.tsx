...
export function RatingBadgeForm({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-6">
      <p className="mb-1 text-sm font-medium text-slate-900">Rating Badge</p>
      ...
      <label className="mb-1 block text-xs text-slate-500">Badge text template</label>
      <input
        type="text"
        value={template}
        onChange={(e) => setTemplate(e.target.value)}
        maxLength={150}
        className="mb-2 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"
      />
      <p className="mb-4 text-xs text-slate-400">
        Default <code className="rounded bg-black/30 px-1 text-emerald-700">{"{rating}"}</code> shows just stars + count.
        Add <code className="rounded bg-black/30 px-1 text-emerald-700">{"{count}"}</code> to include the review number inline.
        Example: <code className="rounded bg-black/30 px-1 text-emerald-700">{"{rating} Based on {count} Reviews"}</code>.
      ...
      {/* Live size preview */}
      <div className="mb-4 flex items-center gap-2 rounded-md border border-slate-200 bg-black/20 px-3 py-2">
