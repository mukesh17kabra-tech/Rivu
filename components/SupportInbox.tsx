    ...
    <main className="flex min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <div className="w-72 flex-shrink-0 border-r border-slate-200 overflow-y-auto">
        <div className="border-b border-slate-200 p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Rivu</p>
          <h1 className="mt-1 text-lg font-semibold">Support Inbox</h1>
        </div>
        {shops.length === 0 ? (
          <p className="p-4 text-sm text-slate-400">No messages yet.</p>
        ) : (
          shops.map((s) => (
            <button
              key={s.shop}
              ...
              className={`block w-full border-b border-slate-100 p-4 text-left transition-colors ${
                selectedShop === s.shop ? "bg-white/[0.05]" : "hover:bg-white"
              }`}
            >
              <div className="flex items-center justify-between">
                <p className="truncate text-sm font-medium text-slate-900">{s.shop}</p>
                {s.unreadCount > 0 && (
      ...
      <div className="flex flex-1 flex-col">
        {!selectedShop ? (
          ...
          <>
            <div className="border-b border-slate-200 p-4">
              <p className="text-sm font-medium text-slate-900">{selectedShop}</p>
            </div>
            ...
            {imageDataUrl && (
              <div className="flex items-center gap-2 border-t border-slate-200 px-4 pt-2">
            ...
            <div className="flex gap-2 border-t border-slate-200 p-4">
              <label className="cursor-pointer text-slate-400 hover:text-slate-900 text-xl self-center">
                📎
